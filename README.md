...
- **エンジンは呼んで使う** — DB や正規表現エンジンを Lean で再実装しない。
  DuckDB や Python をサブプロセスとして裏で走らせ、型付きの JSON でやり取りする
- **8 割で十分** — Python の同等ライブラリと機能を全部そろえることは目指さない。
...
| [`Sys.Regex`](Shed/Sys/Regex.lean) | PCRE 級の正規表現(Python re を呼んで使う) | [tests/RegexTest.lean](tests/RegexTest.lean) |
| [`Sys.Data`](Shed/Sys/Data.lean) | DuckDB を呼んで使う(SQL → 型付き行、一括投入) | [tests/DataTest.lean](tests/DataTest.lean) |
| [`Sys.Py`](Shed/Sys/Py.lean) | Python を呼ぶ逃げ道(型は Lean のまま) | [tests/DataTest.lean](tests/DataTest.lean) |
...
- **`Shed.Sys.*`** — 外部プロセス(curl・python3・DuckDB)をサブプロセスとして
  呼んで使う層。`IO` を返す。外部接続はサブプロセス + JSON が主な経路で、
  FFI は原則使わない

## dbt 連携

データエンジニアリング向けの専用道具。**データの取り決め**(テーブルの列・制約に
ついての約束)を**唯一の定義元**(ここだけで定義し、それ以外は生成物なので手で
編集しない)として管理する。向きは 2 つ:

- **Lean が大もと** — 取り決めを Lean の型で書き、dbt の schema.yml と JSON Schema を
  生成する。[examples/Contracts.lean](examples/Contracts.lean)
- **dbt が大もと** — 既存 dbt プロジェクトの manifest.json をコンパイル時に
...
- [CLAUDE.md](CLAUDE.md) — 開発規約(いま有効なルールの大もと)
- [docs/design.md](docs/design.md) — 設計の経緯(なぜこの設計なのか。追記していく記録)
