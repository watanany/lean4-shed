...
3. `Sys.Data` — DuckDB にパラメータ付き INSERT し、SQL で集計、`queryAs` で型に回収
...
/-- 名前付きグループの値を引く。 -/
def namedStr (m : Regex.Match) (key : String) : Option String :=
...
def parseLine (re : Regex.Re) (line : String) : IO (Option LogRow) := do
  ...
  let some ip := namedStr m "ip" | return none
  let some method := namedStr m "method" | return none
  let some path := namedStr m "path" | return none
  let some status := (namedStr m "status").bind (·.toNat?) | return none
  -- 応答サイズ "-"(本文なし)は 0 と読む
  let bytes := ((namedStr m "bytes").bind (·.toNat?)).getD 0
...
def main (args : List String) : IO Unit := do
  ...
  let (rows, malformed, statusCounts, methodStats, topPaths) ←
    Regex.withRe fun re => Data.withDuck fun db => do
      ...
      for r in rows do
        Log.debug s!"insert: {r.method} {r.path}"
        db.exec "insert into access_log values (?, ?, ?, ?, ?)"
          #[Lean.Json.str r.ip, Lean.Json.str r.method, Lean.Json.str r.path,
            Lean.toJson r.status, Lean.toJson r.bytes]
