...
/-!
# 全モジュールを使う例: アクセスログ・ミニパイプライン

Web サーバーのアクセスログを素材に、shed の道具一式を通しで使う
...
3. `Sys.Data` — DuckDB に `insertRows` で一括投入し、SQL で集計(パラメータ付き)、
   `queryAs` で型に回収
4. `Sys.Py` — 中央値の計算を Python に逃がし、Lean 側の計算と突き合わせる(答え合わせの基準)
5. `Pure.Contract` — 集計テーブルの契約を大もととして定義し、
   実行時検証(許容値チェック)と成果物生成(schema.yml / JSON Schema)の両方に使う
6. `Sys.Subprocess` + `Sys.Http` — 生成した JSON Schema をローカル HTTP サーバーで
   配信し、GET で取り戻して大もとと一致することを確認する(配信して往復で確かめる)
...
def main (args : List String) : IO Unit := do
  ...
  -- 4. Python を答え合わせに: 応答サイズの中央値を両側で計算して突き合わせ
  ...
  check s!"Py 答え合わせ: 中央値が Lean 側と一致({pyMedian} bytes)"
    (pyMedian == leanMedian)
  ...
  -- 6. 配信して往復で確認: 生成物を HTTP で配信し、GET で取り戻して大もとと一致するか
  let serveCmd : Cmd := {
    exe := "python3"
  ...
  try
    ...
    check "HTTP: 配信された JSON Schema が大もとと一致"
