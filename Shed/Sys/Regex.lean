...
/-!
# Shed.Sys.Regex — PCRE 級の正規表現(エンジンの運転)

Python の `re` を常駐ワーカーとして運転する。再実装ではなくエンジンの
運転なので、**先読み・後読み・後方参照・名前付きグループを含む全機能が
最初から使える**(規約「確定需要の原語は一度で完成させる」の充足形。
エンジン = オラクルなので意味論の乖離が原理的に起きない)。
