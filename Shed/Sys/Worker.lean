...
Mutex 取得後から数える)。
-/
def Worker.callJson (w : Worker) (req : Lean.Json) (timeoutSec : Nat := 120) :
    IO Lean.Json :=
      ...
      let exitCode ←
        if timeoutSec == 0 then
          ...
          while result.isNone do
            if let some code ← w.child.tryWait then
              result := some code
            else if (← IO.monoMsNow) ≥ deadline then
              w.child.kill
              result := some (← w.child.wait)
            else
              IO.sleep 10
          pure result.get!
      set (Worker.State.finished exitCode)
