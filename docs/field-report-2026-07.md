# 実地レポート: 全モジュールを使う例を初見で 1 本書いた記録(2026-07)

初見ユーザー(AI エージェント、隔離環境)として shed だけでミニ ETL
(`examples/LogPipeline.lean`)を書き、全モジュールを通過させた記録。
[design.md](design.md) §15「最初に使った場面からの持ち帰り」と同じ趣旨で、引っかかった点を具体的なまま残す。
...
- `examples/LogPipeline.lean` を新規作成: アクセスログを
  ...
  Py を答え合わせの基準にして突き合わせ → 契約から schema.yml / JSON Schema 生成 →
  ローカル HTTP 配信して GET で大もとと一致するか検証。構文エラー 1 件の修正後、初回実行で全緑
- 本筋の流れを README 手順どおり実行: 契約生成 → 実物 dbt 16 項目 PASS →
  manifest をコンパイル時取り込み(`def_dbt_project` / `dbt_check`)→ 全緑。
  異常系テスト(契約外 status 追加)で accepted_values が FAIL 1 になることも確認
...
- **つなぎ目が組み合わせやすい**。`withRe fun re => Data.withDuck fun db => ...` の
...
- **`Py.call "sorted(data)[len(data)//2]" bytesArr` が一行で型付きで返る**。
  逃げ道として過不足がない
- **本筋(契約の中核)は端から端まで本物**。Lean の契約 → 実物 dbt が読む →
  manifest が Lean に戻る → `lake build` が契約の関門になる。この一巡が実際に回り、
  異常系テストも効いた。エラボレーション時 IO の実用例として学びになる
...
2. **`Regex.Match` に名前付きグループを取り出す手段がない**。使う側すべてが
   `m.named.find? (·.1 == key) |>.bind (·.2)` を自作することになる。
   `Match.named? : Match → String → Option String` は収録に値するはず
3. **`Sys.Data` に一括投入の定型がない**。行ごとの `exec insert` は 1 行 = 1 往復。
   23 行なら無問題だが実データでは即詰まる。エンジンを呼んで使う考え方なら
   「一時 CSV/JSON に書いて `create table as select * from 'file'`」が本来のやり方のはずで、
   doc に定型として一行あるだけで違う
4. **「既定で有界」の実装が Http だけ**。Subprocess / Worker / Py / Data は
...
| 対象 | 点 | 一言 |
|---|---|---|
| Pure.Contract | 9 | 小さく正確。生成と実行時検証の両方に同じ大もとが効く |
| Pure.Dbt + Sys.Dbt | 9 | 最も独創的。`lake build` = 契約の関門が実際に回る |
| Sys.Subprocess / Worker | 9 | 土台の信頼感。EPIPE・並行・冪等 shutdown をテストが証明 |
| Sys.Py | 9 | 過不足のない逃げ道 |
| Sys.Regex | 8.5 | 初日から PCRE 全部は正しい。Match の取り出し手段だけ不足 |
| Pure.Glob + Sys.Os | 8.5 | 隠し除外の既定が正しい。`**` も仕様どおり |
| Sys.Http | 8 | 8割用途に十分。本文はテキスト前提 |
| Sys.Log | 7.5 | 説明どおり最小。それ以上でも以下でもない |
| Sys.Data | 7 | queryAs は良い。一括投入の定型と有界性が未整備 |
| セットアップ体験 | 6.5 | nix 経路は頼み綱だが preflight の穴 2 つ |
...
- 「エンジンは呼んで使う」が Regex(re)・Data(DuckDB)・Py で同じ形をしており、
  一つ覚えると全部読める
- Sys.Dbt はエラボレーション時 IO が実務の役に立つ稀有な実例で、
  Lean メタプログラミングの入口として学びになる
- 全公開関数の doc + `#guard` + 使用例の三点セットが「読めば書ける」を実現している。
  実際、本レポートの筆者は module doc と既存の使用例の模倣だけで 1 本書けた

Python と同じ機能を求める人には向かない。それは欠点ではなく仕様(8割/2割)。

## 追記: 改修の持ち帰り(同ブランチ、レポート提出直後)

design.md §15 の流儀で、引っかかった点は指摘で終わらせず即日改修した。

1. **タイムアウト実装(引っかかった点 4 → 解消)**: Subprocess の doc が予告していた
...
2. **`Regex.Match.named?` を収録(引っかかった点 2 → 解消)**: `#guard` example つき。
   LogPipeline の自作ヘルパは削除して置き換え
3. **`Duck.insertRows` を追加(引っかかった点 3 → 解消)**: 一時ファイル +
   `read_json` + `insert into ... by name` の一括投入。キー順不同・クォート・
   日本語・空配列を tests/DataTest.lean で固定。LogPipeline も切り替え
4. **セットアップの穴(引っかかった点 1 → 解消)**: `setup-lean-nix.py` に
   preflight(curl / zstd / xz)を追加し、elan 不在時は `/usr/local/bin` への
   symlink で代替する `register` に分離。elan なしの隔離環境で完走を確認
5. **CLAUDE.md 技術知見に 2 件追記(引っかかった点 6・7)**: 構造体リテラルの
   複数行インデント、`String.trim` deprecated(`trimAscii`)。
   Http.lean に残っていた `String.trim` も取り除いた

これで採点表の主な減点要因(有界性ギャップ、Match の取り出し手段、一括投入、
セットアップ)は解消。残る既知の未実装は「出力サイズ上限」と「結果行数上限」
(実際に必要になるまで待ち。doc に明記済み)。

## 追記 2: 包括レビューの持ち帰り(同ブランチ)
...
- Smoke に退行を捕まえる仕掛け: echo 200 往復 < 1 秒(退化すると 2 秒超で落ちる)
- `insertRows` のエスケープ漏れ: 識別子の `"` とパスの `'` を二重化
  (`"` 入りテーブル名のテストを DataTest に追加)
- 使う例のないタイムアウトの解消: Duck の重いクエリ打ち切り + 打ち切り後は
  その Duck が使えないこと + Py の `time.sleep` 式打ち切りを DataTest で固定
- `parseLine` → `parseLine?`(三段命名違反。examples は規約の見本なので)
...
kill 方式(行プロトコルでは正しい深さ)、`check` ヘルパの各使用例での重複(既存規約)。
